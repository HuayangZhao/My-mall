...
##问题
1.首页制作时掌握小技巧，通过给盒子绝对定位，前提盒子不设宽高，设置top，bottom值限定高度，left，right值限定宽度。四个值一起用可以限定盒子区域。<br>
...
```
4.编辑收货地址和添加收货地址共用同一页面，一开始进入死胡同，一心想通过判断是否携带id来区分，但是通过这种方法是无法实现判断的，因为是同一个按钮，每次点击都是要携带id的。<br>
解决方法：编辑地址和添加地址是点击不同按钮进入的，通过a标签传参的不同完美区分。<br>
